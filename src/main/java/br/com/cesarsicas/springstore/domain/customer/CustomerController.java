...
public class CustomerController {
    ...
    @PostMapping
    ResponseEntity save(@RequestBody CreateCustomerDto customerDto, @AuthenticationPrincipal UserEntity user) {
        customerService.saveCustomer(customerDto, user);
        return ResponseEntity.ok().build();
    }
    @PutMapping
    ResponseEntity update(@RequestBody UpdateCustomerDto updateCustomerDto, @AuthenticationPrincipal UserEntity user) {
